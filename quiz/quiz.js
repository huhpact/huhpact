...
document.addEventListener('DOMContentLoaded', () => {
    totalQuestions = parseInt(sessionStorage.getItem('quizQuestions')) || 10;
    selectedRegion = sessionStorage.getItem('quizRegion') || 'all';
...
function initializeQuiz() {
    ...
    if (selectedRegion !== 'all') {
        availableCountries = countries.filter(country => country.region === selectedRegion);
    }
...
function loadQuestion() {
    ...
    document.getElementById('flag-image').src = currentQuestion.flag;
    document.getElementById('flag-image').alt = `Flag of ${currentQuestion.name}`;
...
function getRandomWrongAnswers(correctCountry) {
    let availableCountries = countries.filter(c => c.name !== correctCountry.name);
