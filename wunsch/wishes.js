/* ==============================================================
   WÜNSCHE
   Jeder Eintrag braucht:

   - title:  Titel des Wunsches
   - image:  Bild-URL (auch eigene Bilder/Pfade möglich, z. B. "bilder/foto.jpg")
   - rating: Wunschgrad von 1 bis 5
   - link:   Link zum Produkt
   - note:   optionale kurze Notiz (darf leer bleiben "")
   ============================================================== */

const WISHES = [
  {
    title: "Datejust 41mm",
    image: "/wunsch/images/datejust.jpg",
    rating: 5,
    link: "https://www.rolex.com/de/watches/datejust/m126334-0013",
    note: ""
  },
  {
    title: "Ralph Lauren USA Sweater",
    image: "/wunsch/images/rl-usa.webp",
    rating: 4,
    link: "https://www.ralphlauren.de/de/der-klassische-flaggenpullover-3616854058028.html",
    note: ""
  },

  {
    title: "Apple Vision Pro",
    image: "/wunsch/images/avp.jpg",
    rating: 3,
    link: "https://www.apple.com/de/shop/buy-vision/apple-vision-pro",
    note: ""
  },
  {
    title: "Osmo Pocket 4",
    image: "/wunsch/images/dji.webp",
    rating: 3,
    link: "https://store.dji.com/de/product/osmo-pocket-4?vid=218422",
    note: ""
  },
  {
    title: "Anker 165W PowerBank",
    image: "/wunsch/images/anker.webp",
    rating: 2,
    link: "https://www.anker.com/de/products/a1695-anker-powerbank-25000mah-165w",
    note: ""
  },
  {
    title: "Cloudtilt On",
    image: "/wunsch/images/on.webp",
    rating: 3,
    link: "https://www.farfetch.com/fr/shopping/men/on-baskets-cloudtilt-blackivory-item-24184032.aspx",
    note: ""
  },
  {
    title: "Oral B IO 10",
    image: "/wunsch/images/oralb.webp",
    rating: 3,
    link: "https://www.oralb.de/de-de/produkte/elektrische-zahnbuerste/oral-b-io-10-elektrische-zahnbuerste-reise-etui?variation=13936276",
    note: ""
  },
  {
    title: "Ami Paris Sweatshirt",
    image: "/wunsch/images/ami.webp",
    rating: 3,
    link: "https://www.farfetch.com/fr/shopping/men/ami-paris-pull-ami-de-coeur-en-maille-intarsia-item-30651156.aspx",
    note: ""
  },
   {
    title: "Steven 8479",
    image: "/wunsch/images/pri.webp",
    rating: 3,
    link: "https://premiata.eu/collections/steven/products/ste08479",
    note: ""
  },
   {
    title: "Cabin Plus",
    image: "/wunsch/images/rimowa.avif",
    rating: 2,
    link: "https://www.rimowa.com/de/de/luggage/colour/silver/cabin-plus/92556004.html",
    note: ""
  },
  {
    title: "Alpha 7",
    image: "/wunsch/images/alpha3.jpg",
    rating: 3,
    link: "https://www.sony.de/electronics/alpha7",
    note: ""
  },
  
];
